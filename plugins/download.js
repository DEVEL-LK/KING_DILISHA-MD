...
async (conn, mek, m, { from, q, reply, l }) => {
    try {
        ...
        let txt = `*👑 LOAD DILISHA G DRIVE CENTER 👑*
...
async(conn, mek, m, {
  ...
  try {
    ...
    let caption = `\`🎧 *LOAD DILISHA SONG CENTER* 🎧\`	
*┌────────────────────┐*
*├ \`🎶 Title\` : ${result.title}*
*├ \`🐼 Views\` : ${result.views}*
...
async (conn, mek, m, {
  ...
  try {
    ...
    let caption = `🎶 *LOAD DILISHA SONG DOWNLOADER* 🎶
...
let caption = `*\`📥 LOAD DILISHA TIKTOK SHOP 📥\`*
...
cmd({
    ...
    try {
        ...
        const caption = `\`📥 LOAD DILISHA FB CENTER 📥\`\n\n` +
                   `*┌──────────────────*\n` +
...
async(conn, mek, m,{from, q, reply}) => {
  try {
    ...
    let listdata = `*\`📥 LOAD DILISHA APK STORE 📥\`*
...
try{            
    ...
    let wm = config.FOOTER
    var result = results.videos[0]
     let caption = `*📁 LOAD DILISHA VIDEO STORAGE 📁*
*┌─────────────────────┐*
*├ \`📹 Title\` : ${result.title}* 
*├ \`🐼 Views\` : ${result.views}*
*├ \`⌛Duration\` : ${result.duration}*
...
let listdata = `*\`🔥 LOAD DILISHA MEDIA FIRE 🔥\`*
...
let msg = `*\`🔞 LOAD DILISHA PORN HUB WEB 🔞\`*
...
async (conn, mek, m, { from, prefix, l, quoted, body, isCmd, command, args, q, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins, reply }) => {
    try {
        ...
        let caption = `*\`🎼 LOAD DILISHA SPOTIFY HUB 🎼\`*
*┌──────────────────╮*
*├ \`🎶 Title:\`* ${data.title}
*├ \`🧑‍🎤 Artist:\`* ${data.artist}
*├ \`💽 Album:\`* ${data.album}
...
async (conn, mek, m, { from, prefix, q, reply }) => {
    try {
        ...
        const buttonMessage = {
            ...
            caption: caption,
            footer: config.FOOTER || 'LOAD_DILISHA',
            buttons: buttons,
            headerType: 4
        };
...
_🎧 Powered by LOAD DILISHA Music_`;
